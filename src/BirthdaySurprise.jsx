...
/* ─────────────────────────────────────
   LOCAL STORAGE KEYS
───────────────────────────────────── */
const LOCAL_DATA_KEY  = "bday_all_submissions";   // permanent local backup
const EMAIL_QUEUE_KEY = "bday_email_retry_queue"; // retry queue for failed emails

/* ─────────────────────────────────────
   LOCAL BACKUP — always saved first
───────────────────────────────────── */
const saveToLocal = (type, value) => {
  try {
    const existing = JSON.parse(localStorage.getItem(LOCAL_DATA_KEY) || "[]");
    existing.push({ type, value, time: new Date().toISOString() });
    localStorage.setItem(LOCAL_DATA_KEY, JSON.stringify(existing));
  } catch (e) {}
};

/* ─────────────────────────────────────
   EMAIL QUEUE — retry storage
───────────────────────────────────── */
const addToQueue = (type, value) => {
  try {
...
/* ─────────────────────────────────────
   SEND ONE EMAIL (Promise-based)
───────────────────────────────────── */
const sendEmailNow = (type, value, time) => {
  return new Promise((resolve, reject) => {
    try {
      if (!window.emailjs) { reject(new Error("emailjs not loaded")); return; }
      // Re-init before every send to avoid stale auth state
...
const processEmailQueue = async () => {
  try {
    ...
    for (const item of queue) {
      try {
        await sendEmailNow(item.type, item.value, item.time);
        // sent OK — drop from queue
      } catch {
...
/* ─────────────────────────────────────
   MAIN SEND — save locally FIRST, then email; if email fails → queue it
───────────────────────────────────── */
const sendToOwner = (type, value) => {
  // 1. Always persist locally regardless of email
  saveToLocal(type, value);

  // 2. Try to send email immediately
  sendEmailNow(type, value).catch(() => {
    // 3. If it fails, add to retry queue
    addToQueue(type, value);
  });
};

/* ─────────────────────────────────────
   OWNER LOG (session-level activity log)
───────────────────────────────────── */
const ownerLog = {
  add: (type, value) => {
    try {
  ...
  get:   () => { try { return JSON.parse(sessionStorage.getItem("ownerLog") || "[]"); } catch { return []; } },
...
const useTyping = () => {
  ...
  const type = useCallback((setter, text, opts = {}, cb) => {
    ...
    const step = () => {
      ...
      if (i < text.length) { acc += text[i++]; setter(acc); setTimeout(step, charDelay); }
...
const CelebrationStickers = ({ type, onDone }) => {
  ...
  const slideStyle = (side) => {
    ...
    return {
      position: "fixed", bottom: "5%", [side]: 0, zIndex: 7500, pointerEvents: "none",
  ...
  const bannerStyle = {
    position: "fixed", top: "10%", left: "50%", zIndex: 7500, pointerEvents: "none",
    display: "flex", flexDirection: "column", alignItems: "center", gap: "6px",
    transform: phase === 1 ? "translateX(-50%) scale(1)" : "translateX(-50%) scale(0)",
    transition: phase === 2 ? "transform 0.4s ease-in" : phase === 1 ? "transform 0.6s cubic-bezier(.34,1.56,.64,1)" : "none",
    opacity: phase === 1 ? 1 : 0
  };
  return (<>
...
const AnteIshtheScreen = ({ onNext }) => {
  ...
  const submit = () => {
    ...
    sendToOwner("💬 Ante Isthe Reply", val);
...
const NameEntryScreen = ({ onSuccess, triggerBurst }) => {
  ...
  const closeHint = () => {
    ...
    sendToOwner("💡 Hint Screen Reply", reply);
...
const MessageScreen = ({ onNext }) => {
  ...
  const handleKittyTap = () => {
    ...
    if (tapCount.current >= 5) {
      tapCount.current = 0; setEasterText(""); setEasterBtn(false); setShowEaster(true); playPop();
  ...
  const handleStart = () => {
    ...
    sendToOwner("🎂 Birthday Message Reaction", val);
    playPop(); setTimeout(() => onNext(), 300);
  };
  return (
    <Screen>
...
const BirthdayScreen = ({ onNext }) => {
  ...
  const handleNext = () => {
    ...
    sendToOwner("🎉 Happy Birthday Screen Feeling", val);
    playPop(); setTimeout(() => onNext(), 300);
  };
  return (
...
const GalleryScreen = ({ onNext }) => {
  ...
  const saveCurrent = () => {
    ...
    sendToOwner(`📸 Photo ${galIndex + 1} Feeling — ${PHOTOS[galIndex].label}`, val);
    playPop();
    setTimeout(() => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", padding: "5px 12px 10px", animation: "screenFadeIn .6s ease forwards" }}>
      <h2 style={{ fontFamily: "'Great Vibes',cursive", color: "#d1006f", fontSize: "clamp(13px,3.2vw,20px)", margin: "0 0 4px", flexShrink: 0, lineHeight: 1.1, textShadow: "0 0 8px rgba(255,100,160,.4)" }}>{title}</h2>
      <div style={{ position: "relative", width: "min(190px,48vw)", aspectRatio: "3/4", flexShrink: 0, margin: "0 auto 4px", perspective: "700px" }}
        onTouchStart={e => { touchStart.current = e.touches[0].clientX; }}
        onTouchEnd={e => { const dx = e.changedTouches[0].clientX - touchStart.current; if (Math.abs(dx) > 40) { dx < 0 ? next() : prev(); } }}
      >
        {PHOTOS.map((photo, i) => (
          <div key={i} style={{ position: "absolute", inset: 0, background: "#fff", borderRadius: "10px", padding: "5px 5px 24px", boxShadow: "0 6px 22px rgba(180,60,120,.18)", transition: "transform .42s cubic-bezier(.34,1.5,.64,1),opacity .32s ease", willChange: "transform,opacity", cursor: i === galIndex ? "pointer" : "default", ...cardStyle(i) }}
            onClick={() => handlePhotoTap(i)}
          >
            {saved[i] && (<div style={{ position: "absolute", top: "4px", right: "4px", width: "18px", height: "18px", borderRadius: "50%", background: "linear-gradient(135deg,#4caf50,#2e7d32)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 20, fontSize: "10px", boxShadow: "0 2px 5px rgba(0,0,0,.2)" }}>✓</div>)}
            {i === galIndex && (<div style={{ position: "absolute", top: "4px", left: "4px", zIndex: 20, background: "rgba(255,255,255,0.75)", borderRadius: "8px", padding: "2px 5px", fontFamily: "'Caveat',cursive", fontSize: "9px", color: "#c060a0", lineHeight: 1.2, pointerEvents: "none" }}>3× tap 💗</div>)}
            <div style={{ width: "100%", height: "calc(100% - 24px)", borderRadius: "6px", overflow: "hidden", background: "#fff8fc", position: "relative" }}>
              <img src={photo.src} alt={photo.label} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 15%", display: "block", transition: "filter 1.2s", filter: i === galIndex ? "saturate(1) brightness(1)" : "saturate(.12) brightness(1.4)" }} onError={e => { e.target.style.display = "none"; }} />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "5px", marginBottom: "3px", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: "2px", alignItems: "center", maxWidth: "52vw", overflow: "hidden" }}>
          {PHOTOS.map((_, i) => (<div key={i} onClick={() => { if (!isAnimating) { setIsAnimating(true); setGalIndex(i); startAuto(); setTimeout(() => setIsAnimating(false), 400); } }} style={{ width: i === galIndex ? "13px" : "4px", height: "4px", borderRadius: "3px", background: saved[i] ? "#4caf50" : i === galIndex ? "#e040a0" : "#f4a0c8", transition: "all .3s cubic-bezier(.34,1.5,.64,1)", cursor: "pointer", flexShrink: 0 }} />))}
        </div>
      ...
      <div style={{ display: "flex", gap: "5px", alignItems: "center", width: "min(340px,90vw)", flexShrink: 0, marginBottom: "4px" }}>
        <input key={galIndex} value={draft} onChange={e => handleDraftChange(e.target.value)}
          ...
          style={{ flex: 1, border: `2px solid ${inputShake ? "#e040a0" : saved[galIndex] ? "#4caf50" : "#f4a0c8"}`, borderRadius: "14px", padding: "9px 11px", fontFamily: "'Caveat',cursive", fontSize: "clamp(13px,3vw,16px)", fontWeight: 700, color: "#d8368e", background: "#fff5fa", outline: "none", textAlign: "center", boxSizing: "border-box", transition: "border-color .25s", animation: inputShake ? "shakeInput .4s ease" : "none" }}
        />
        <button onClick={saveCurrent} style={{ flexShrink: 0, width: "34px", height: "34px", borderRadius: "50%", border: "none", background: draft.trim() ? "linear-gradient(135deg,#f080b8,#e040a0)" : "#f4d0e4", cursor: draft.trim() ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", boxShadow: draft.trim() ? "0 3px 10px rgba(220,70,140,.3)" : "none", transition: "all .2s" }}>
          {saved[galIndex] ? "💾" : "💖"}
        </button>
        <button onClick={handleNext} style={{ flexShrink: 0, height: "34px", padding: "0 12px", borderRadius: "17px", border: "none", cursor: "pointer", fontFamily: "'Caveat',cursive", fontSize: "clamp(12px,2.8vw,15px)", fontWeight: 800, color: "white", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: "4px", background: canProceed ? "linear-gradient(135deg,#f080b8,#e040a0)" : "linear-gradient(135deg,#f9c0d8,#f4a8c4)", boxShadow: canProceed ? "0 4px 14px rgba(220,70,140,.4)" : "0 2px 8px rgba(220,70,140,.15)", transition: "all .25px" }}>
      ...
      {secretMsg && (
        <div onClick={() => setSecretMsg(null)} style={{ position: "fixed", inset: 0, background: "rgba(240,80,160,.22)", backdropFilter: "blur(10px)", zIndex: 9100, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: "28px", padding: "28px 24px", maxWidth: "340px", width: "88vw", boxShadow: "0 10px 48px rgba(220,70,140,.35)", border: "2.5px solid #f9c0d8", display: "flex", flexDirection: "column", alignItems: "center", gap: "12px", animation: "popIn .4s cubic-bezier(.34,1.56,.64,1)" }}>
            ...
            <button onClick={() => setSecretMsg(null)} style={{ background: "linear-gradient(135deg,#f080b8,#e040a0)", border: "none", borderRadius: "20px", padding: "10px 24px", color: "white", fontFamily: "'Caveat',cursive", fontSize: "16px", fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 14px rgba(220,70,140,.35)" }}>💖 Aww!</button>
          </div>
        </div>
      )}
      {lightbox && (
...
const LetterScreen = ({ onNext }) => {
  ...
  const handleBowTap = () => {
    ...
    if (n >= 5) {
      setBowTaps(0); setBowEaster(true); setBowText(""); setBowBtn(false); playPop();
    ...
    <Screen style={{ justifyContent: sealBroken ? "flex-start" : "center", paddingTop: "12px", paddingBottom: "12px" }}>
      ...
      {sealBroken && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 0, width: "100%", height: "100%", padding: "8px 0", animation: "fadeInUp .5s ease" }}>
          ...
          <div onClick={handleBowTap} style={{ fontSize: "20px", margin: "2px 0 6px", flexShrink: 0, cursor: "pointer", userSelect: "none", transition: "transform .15px" }} title="Tap 5x for a secret 💗">💗</div>
...
const CreditsScreen = ({ onRestart }) => {
  ...
  const saveFeeling = () => {
    ...
    sendToOwner("💖 Final Feeling (Credits Screen)", val);
...
const OwnerPanel = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    // Load local backup
    try {
      const ld = JSON.parse(localStorage.getItem(LOCAL_DATA_KEY) || "[]");
      setLocalData(ld);
    } catch (e) {}
    return () => clearInterval(iv);
  }, []);
  ...
  const SHOWN = new Set(["name_correct", "name_wrong", "hint_opened", "hint_reply", "ante_isthe_reply", "message_reaction", "birthday_reaction", "credits_feeling", "easter_egg", "slide_time", "photo_feeling", "photo_secret"]);
  ...
  // Retry queued emails manually
  const retryNow = () => { processEmailQueue(); };
  const queueCount = (() => { try { return JSON.parse(localStorage.getItem(EMAIL_QUEUE_KEY) || "[]").length; } catch { return 0; } })();
      ...
      <div style={{ background: "linear-gradient(160deg,#fff5fb,#ffe8f4)", border: "2.5px solid #f4a0c8", borderRadius: "28px", padding: "20px", maxWidth: "500px", width: "96vw", maxHeight: "90vh", display: "flex", flexDirection: "column", gap: "12px", boxShadow: "0 12px 56px rgba(220,70,140,.22)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
          <div>
            ...
            <div style={{ fontFamily: "'Caveat',cursive", fontSize: "clamp(11px,2.5vw,13px)", color: "#c090a0", marginTop: "2px" }}>
              🎵×5 to open · {queueCount > 0 ? `⚠️ ${queueCount} emails pending` : "✅ email queue clear"}
            </div>
          </div>
          <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
            {queueCount > 0 && <button onClick={retryNow} style={{ background: "linear-gradient(135deg,#34d399,#059669)", border: "none", color: "white", borderRadius: "14px", padding: "7px 12px", cursor: "pointer", fontFamily: "'Caveat',cursive", fontSize: "clamp(12px,2.5vw,14px)", fontWeight: 700 }}>🔄 retry</button>}
        ...
        <div style={{ display: "flex", gap: "4px", background: "rgba(244,160,200,0.15)", borderRadius: "24px", padding: "4px", flexShrink: 0 }}>
          ...
          {TAB("backup", `💾 Backup(${localData.length})`, activeTab === "backup")}
        </div>
        {activeTab === "story" && (
        ...
        {activeTab === "backup" && (
          <div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "8px", paddingRight: "2px" }}>
            <p style={{ fontFamily: "'Caveat',cursive", fontSize: "13px", color: "#c090a0", textAlign: "center", margin: 0 }}>💾 localStorage backup — persists across sessions</p>
            {localData.length === 0
              ? <div style={{ textAlign: "center", padding: "32px", fontFamily: "'Caveat',cursive", color: "#c070a0", fontSize: "18px" }}>No local data yet 💗</div>
              : [...localData].reverse().map((entry, i) => (
                <div key={i} style={{ background: "#f0fff4", borderRadius: "14px", padding: "10px 14px", border: "1.5px solid #4caf5033" }}>
                  <div style={{ display: "flex", gap: "6px", alignItems: "center", marginBottom: "4px" }}>
                    <span style={{ fontFamily: "'Caveat',cursive", fontSize: "12px", color: "#2e7d32", fontWeight: 700 }}>{entry.type}</span>
                    <span style={{ fontFamily: "'Caveat',cursive", fontSize: "10px", color: "#90a090", marginLeft: "auto" }}>{new Date(entry.time).toLocaleString()}</span>
                  </div>
                  <div style={{ fontFamily: "'Caveat',cursive", fontSize: "clamp(14px,3vw,17px)", color: "#1b5e20", wordBreak: "break-word" }}>{entry.value}</div>
                </div>
              ))
            }
          </div>
...
export default function BirthdaySurprise() {
  ...
  // ── On mount: process retry queue immediately + every 3 minutes ──
  useEffect(() => {
    // Small delay so emailjs script has time to load
    const t1 = setTimeout(() => processEmailQueue(), 4000);
    const interval = setInterval(() => processEmailQueue(), 3 * 60 * 1000);
    return () => { clearTimeout(t1); clearInterval(interval); };
  }, []);

  // ── Also retry when the tab becomes visible again ──
  useEffect(() => {
    const onVisible = () => { if (!document.hidden) processEmailQueue(); };
